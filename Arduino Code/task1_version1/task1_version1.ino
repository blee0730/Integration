...
void loop(){
  ...
  if(distance == 10 || distance == 9){
    ...
    delay(4200); //rotate until it is facing the other way (180 degrees)
    stop();
    movFW(100); //move forward until the ground sensors see green to start the search function
  }
  if(LCol == 3 && RCol == 3){
    stop();
    search();
...
void readColorSensors() 
{
  ...
  if((RblueCol <= 30) && (RredCol <= 30) && (RgreenCol >= 0 && RgreenCol <= 100))
  {
...
void search(){
  ...
  if(MCol == 2){
    ...
    if(RCol == 2); //if the right color sensor detects blue stop align car to the center of the line
    stop();
    movRT(100);
    delay(100);
  }
...
void follow(){
  ...
  if(MCol == 2 && RCol == 2 && LCol == 2 && Size == 0){ //placeholder for Size
    stop();
    movLFT(100);

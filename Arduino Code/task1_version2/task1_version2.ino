...
void loop(){
  ...
  if((distance == 10 || distance == 9) && grabbed == false){
    stop();
    grab(); //grab the box
    grabbed = true;
    readColorSensors();
    movBW(100); //move back so the ultrasonic sensor no longer sees 10
    delay(300); //move back until the vehicle is sufficiently away from the platform
    stop();
    if(MCol == 1){
      movCW(100); //rotate so the vehicle is facing the right way
      delay(4300); //rotate until it is facing the other way (180 degrees)
      stop();
    }
    if(MCol == 2){
      movCCW(100); //rotate so the vehicle is facing the right way
      delay(4300); //rotate until it is facing the other way (180 degrees)
      stop();
    }
  }
  if(LCol == 3 && RCol == 3 && grabbed == true){
    stop();
    movFW(100);
    delay(500);
    while(searching == true && grabbed == true){
      search();
    }
  }
  while(searching == false && done == false){
    follow();
  }
  while(done == true){
    finish();
...
void readColorSensors() 
{
  ...
  if((MblueCol <= 150) && (MredCol >= 100 && MredCol <= 250) && (MgreenCol <= 140))
  {
  ...
  if((LblueCol >= 150 && LblueCol <= 230) && (LredCol <= 40) && (LgreenCol <= 170))
  {
  ...
  if((MblueCol >= 150 && MblueCol <= 250) && (MredCol <= 70) && (MgreenCol <= 160))
  {
...
void grab(){
  for(int i = 0; i <= 50; i++)
  {
    ...
    {    
      ...
      Gripper.write(GripperPosition-1); //setting the gripper position to one less to grab the box tighter so it doesn't slip down
      if(GripperPosition >= 120 && GripperPosition <= 131)
      {
        Size = 1;
      }

      if(GripperPosition >= 132 && GripperPosition <= 160)
      {
        Size = 0;
      }
...
void search(){
  ...
  if(MCol == 1){ //if the box is red look for the red line
    stop();
    movLFT(100); //direction depends on where the red line is
    if(LCol == 1 && RCol == 0){ //if the left color sensor detects red and right color detects black stop align car to the center of the line
      stop();
      searching = false;
    }
  }
  if(MCol == 2){
    stop();
    movRT(100); //direction depends on color
    if(LCol == 0 && RCol == 2){ //if the left color sensor detects black and right color detects blue stop align car to the center of the line
      stop();
      searching = false;
    }
  }
...
void follow(){
  ...
  movFW(100);
  //corrects the vehicle to recenter on the line
  correctLine();
  ...
  if(MCol == 1 && RCol == 1 && diag == true){
    // stop();
    movDiagRT(100);
  }
  //if the middle sensor detects blue follow the blue line
  if(MCol == 2 && LCol == 2 && diag == true){
    // stop();
    movDiagLFT(100);
  }
  //if all sensors read red and the size is big move left
  if(MCol == 1 && RCol == 1 && LCol == 1 && Size == 0){
    // stop();
    movLFT(100);
    diag = false;
  }
  //if the left sensor sees black and the right sees red go forward
  if(RCol == 1 && LCol == 0 && MCol == 1 && Size == 0 && diag == false){
    // stop();
    movFW(100);
  }
  //if all sensors read red and the size is small turn right
  if(MCol == 1 && RCol == 1 && LCol == 1 && Size == 1){
    // stop();
    movRT(100);
    diag = false;
  }
  //if the right sensor sees black and the left sees red go forward
  if(RCol == 0 && LCol == 1 && MCol == 1 && Size == 1 && diag == false){
    // stop();
    movFW(100);
  }
  ...
  //if the left sensor sees black and the right sees blue go forward
    if(RCol == 2 && LCol == 0 && MCol == 2 && Size == 0 && diag == false){
      // stop();
      movFW(100);
    }
  //if all sensors read blue and the size is small turn right
  if(MCol == 2 && RCol == 2 && LCol == 2 && Size == 1){
    // stop();
    movRT(100);
    diag = false;
  }
  //if the right sensor sees black and the left sees blue go forward
  if(RCol == 0 && LCol == 2 && MCol == 2 && Size == 1 && diag == false){
    // stop();
    movFW(100);
  }
  // if(RCol == 3 && LCol == 3){
...
void correctLine(){
  if((RCol == 1 && LCol == 0) || (RCol == 2 && LCol == 0)){
    stop();
    movRT(100);
  }
  if((LCol == 1 && RCol == 0) || (LCol == 2 && RCol == 0)){
    stop();
    movLFT(100);
  }
}
